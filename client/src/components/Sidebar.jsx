...
const Sidebar = () => {
  ...
  // Add to your existing useState import
  const [menuOpen, setMenuOpen] = useState(false);

  return (
    <div className="bg-black/90 h-full !p-5 overflow-y-scroll text-white">
      {/* Header */}
      <div className="flex items-center !py-2 justify-between">
        ...
        <div className="relative px-3">
          ...
          {menuOpen && (
            <div
              className="absolute right-0 top-full mt-2 w-40 rounded-sm bg-black text-white shadow-xl 
                  transition-all duration-200 px-2 z-50"
            >
              <p
                onClick={() => {
                  navigate("/profile");
      ...
      {/* Search */}
      <div className="gap-2 !py-3 !px-4 flex items-center border border-white/10 rounded-lg mt-4">
        ...
        {filteredUsers.map((user) => {
          ...
          return (
            ...
            >
              ...
              {/* Unseen badge */}
              {unseenMessages[user._id] > 0 && (
                <span className="ml-auto shrink-0 text-[10px] !px-2 !py-0.5 rounded-full bg-green-500 text-white font-semibold">
                  {unseenMessages[user._id]}
                </span>
              )}
            </div>
